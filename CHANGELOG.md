...
- Upgraded `@j1nn0/herdr-plugin-sdk` to `^0.3.0` and migrated the inbox pane-open command to the typed `client.plugin.pane.open()` API.
...
- Result Inbox now uses a simpler default layout and supports optional JSON field visibility and ordering configuration.

## [0.5.0] - 2026-09-16

### Added

- Result Inbox rows now surface captured pane and claimed-task context so agent work can be identified without opening each Result.
