...
import { normalizeSubmitPayload } from "./collector-contract.ts";
import { readHookStdin, runCodexHookEvent, writeFailure } from "./hook-runtime.ts";

export async function runSubmitHook(): Promise<number> {
  let json: string;
  try {
  ...
  const normalized = normalizeSubmitPayload(parseJson(json));
  if (!normalized.ok) {
    return 0;
  }
...
if (isMainModule()) {
  process.exitCode = await runSubmitHook();
}
